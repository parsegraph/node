 ...
 */
  animate(t:number) {
    ...
    const x = lerp(this._cameraPosition.cameraX-cam.width()/(2*this._cameraPosition.scale), -nodePosition.x, t);
    const y = lerp(this._cameraPosition.cameraY-cam.height()/(2*this._cameraPosition.scale), -nodePosition.y, t);
    const scale = lerp(this._cameraPosition.scale,
      Math.min(this.getFocusedCameraScale(), this.getRequiredScale()),
      t);
    cam.setOrigin(x + cam.width()/(2*scale), y + cam.height()/(2*scale));
    cam.setScale(scale);
  }
