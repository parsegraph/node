...
export default class LispCell {
  ...
  eval(env:LispEnvironment):LispCell {
      ...
      if(proc.type == LispType.Lambda) {
          ...
          return proc.list[2].eval(new LispEnvironment(
              proc.env, proc.list[1].list, exps
          ));
      }
...
const lisp_false_sym = lisp_dialect === lisp_SCHEME ? new LispCell(LispType.Symbol, "#f") : lisp_nil;
