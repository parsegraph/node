  ...
  drawLine(direction: Direction, node: Node<DefaultNodeType>) {
    ...
    if (isVerticalDirection(direction)) {
      const length =
        ...
        parentScale *
        (directionData.lineLength - node.size().height() / 2);
      painter.drawBlock(
        node.groupX(),
        node.groupY() +
          length / 2 +
          (parentScale * directionSign(direction) * node.size().height()) / 2,
        thickness,
        Math.abs(length),
        0,
        0
      ...
      const length =
        directionSign(direction) *
        parentScale *
        (directionData.lineLength - node.size().width() / 2);
      painter.drawBlock(
        node.groupX() +
          length / 2 +
          (parentScale * directionSign(direction) * node.size().width()) / 2,
        node.groupY(),
        Math.abs(length),
        thickness,
        0,
